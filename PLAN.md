...
| Ring capacity      | 10 entries; 11th append auto-clears the ring so new entry becomes entry 1       |
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `feat/opt-e-erase` (merged)      | **Paste on Option release** — removed auto-dismiss timer from CycleHUD; paste fires when user releases the Option modifier key. `HotkeyEngine` gained `.flagsChanged` monitoring and `onOptionReleased` callback; `AppController` wires it to `hud.commitPaste()`. |
| `feat/opt-e-erase` (merged)      | **Option+E — erase all history** — new global hotkey (keyCode 14); fires at any time (cycling or not); resets `isCycling`; wired to `AppController.clearHistory()`.                                                                                                |
| `feat/opt-e-erase` (merged)      | **Disable App Sandbox** — `ENABLE_APP_SANDBOX = YES` was silently blocking all `CGEventTap` creation. Set `ENABLE_APP_SANDBOX = NO` in both Debug and Release.                                                                                                     |
| `feat/opt-e-erase` (merged)      | **Prevent silent paste after erase** — `AppController.commitPaste()` now guards on `hud.isVisible`; if the HUD is hidden (e.g. cleared by Option+E mid-cycle), releasing Option is a no-op.                                                                        |
| `feat/hud-styling` (merged)      | **HUD restyled** — `.menu` material; `NSColor.controlAccentColor` counter; SF system font; 12pt corner radius; 0.5pt white border; no panel shadow; counter top-left; HUD appears below-right of cursor; 100ms fade-in / 80ms fade-out; badge panel removed.       |
| `feat/hud-styling` (merged)      | **Xcode Canvas previews** — `CycleHUD+Preview.swift` with three `#Preview` variants (`#if DEBUG` only).                                                                                                                                                            |
| `feat/following-blinker`         | **Green menu bar icon** — `MenuBarExtra` label closure with `Image.foregroundStyle(.green)`.                                                                                                                                                                        |
| `feat/following-blinker`         | **Ring capacity = 10** — `ClipboardRing.capacity`; 11th append auto-clears, new entry becomes entry 1.                                                                                                                                                             |
| `feat/following-blinker`         | **3-level AX caret detection** — `CycleHUD.repositionNearCursor()` tries: (1) `kAXBoundsForRangeParameterizedAttribute` on focused element (native Cocoa apps, pixel-perfect); (2) same on first text-editing child element; (3) focused element frame bottom-left (Electron, browsers, Terminal); falls back to mouse offset. Proper `AXValueGetType` guards; 1-char range fallback for bare insertion points. |
| `feat/following-blinker-v2`      | **Active branch** — current development.                                                                                                                                                                                                                            |
